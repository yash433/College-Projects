...
public class ScenarioAnalysis {
    ...
    public void computeCO2EmissionsAndCost () {
      ...
      for (int i = 0; i < vehicles.length; i++){
	              ...
	              double otherCost = 0;
	              double fuelCost = 0;
	              double milesAllowed = 0;

	              if (vehicles[i].getFuel().getType() == vehicles[i].getFuel().GAS){
	                  milesAllowed = (double)(vehicles[i].getLease().getMileageAllowance()*(vehicles[i].getLease().getNumberOfMonths()/12));

	                  co2 = (milesAllowed/vehicles[i].getFuel().getUsage()) * Fuel.CO2EMITTED_GASCOMBUSTION;
	                  vehicles[i].setCO2Emission(co2);

	                  fuelCost = getGasPrice()*(milesAllowed / vehicles[i].getFuel().getUsage());
	                  otherCost = vehicles[i].getOtherCost()+vehicles[i].getLease().getDueAtSigning()+(vehicles[i].getLease().getNumberOfMonths()*vehicles[i].getLease().getMonthlyCost());
...
if (vehicles[i].getFuel().getType() == vehicles[i].getFuel().ELECTRIC){
milesAllowed = (double)(vehicles[i].getLease().getMileageAllowance()*(vehicles[i].getLease().getNumberOfMonths()/12));

co2 = ((milesAllowed / vehicles[i].getFuel().getUsage())*vehicles[i].getFuel().getKWhPerCharge())*(vehicles[i].getFuel().CO2EMITTED_GENERATEmWh * (0.45/1000));
vehicles[i].setCO2Emission(co2);

fuelCost = ((milesAllowed / vehicles[i].getFuel().getUsage())*vehicles[i].getFuel().getKWhPerCharge())*(getElectricityPrice() / 100);
vehicles[i].setFuelCost(fuelCost);
